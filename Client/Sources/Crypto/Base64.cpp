...
namespace Crypto
{
	...
	string Base64Decode(string const& encoded_string) {
		...
		while (in_len-- && (encoded_string[in_] != '=') && IsBase64(encoded_string[in_])) {
			...
			if (i == 4) {
				for (i = 0; i < 4; i++)
					char_array_4[i] = static_cast<unsigned char>(base64Characters.find(char_array_4[i]));
		...
		if (i) {
			...
			for (j = 0; j < 4; j++)
				char_array_4[j] = static_cast<unsigned char>(base64Characters.find(char_array_4[j]));
